        ...
        Do While True
            ...
            For Each objeto_retornado In json_obj("retorno")("produtos")
                ...
                For Each deposito In produto("depositos")
                    If deposito("deposito")("saldo") <> 0 Then
                        If .Cells(ult_linha, 18).Value <> "" Then .Cells(ult_linha, 18).Value = .Cells(ult_linha, 18).Value & " | " & deposito("deposito")("nome")
                        If .Cells(ult_linha, 18).Value = "" Then .Cells(ult_linha, 18).Value = .Cells(ult_linha, 18).Value & deposito("deposito")("nome")
                    End If
                Next
...
Sub drop_produtos()
    Sheets("BASE_PRODUTOS").Range("A6:R 1048576").Delete
End Sub