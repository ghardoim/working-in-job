...
Private Sub preenche_template(nome_planilha As String)
    ...
    Sheets("final").Range("A1:E" & Sheets("final").Range("A1048576").End(xlUp).Row).ExportAsFixedFormat Filename:=ThisWorkbook.Path & "\Relat�rio - " & nome_planilha & ".pdf", Type:=xlTypePDF
    Sheets("main").Select
